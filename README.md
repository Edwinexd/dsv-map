...
- `floor_plan.png` - Base floor plan image (3056×3056px)
...
- 🗺️ Interactive map with staff profile pictures
- 🔍 Search by name
...
- 📺 TV-optimized PNG output with smart label placement
...
├── get_all_dsv_employees.py         # Scrape employees from Daisy
├── download_all_dsv_pictures.py     # Download profile pictures
├── fix_all_dsv_names.py             # Fix employee names
├── scrape_units.py                  # Scrape unit information
├── create_tv_simple_smart.py        # Generate TV images
├── requirements.txt                 # Python dependencies
├── floor_plan.png                   # Base floor plan (in git)
├── room_positions_easyocr.json      # Room coordinates (in git)
├── manual_positions.json            # Manual overrides (in git)
├── zone_coordinate_picker.html      # Zone picker tool (in git)
└── output/                          # Generated files (gitignored)
    ├── html/
    │   └── staff_map_unified.html
    └── tv/
        ├── all_dsv_staff_map_tv.png
        ├── ACT_map_tv.png
        ├── Cen_map_tv.png
        └── ... (one per unit)
```
