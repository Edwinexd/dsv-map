...
def main(employee_json, output_png, title=None):
    ...
    with open(room_positions_file, encoding="utf-8") as f:
        ocr_rooms = json.load(f)

    # Zone coordinates
    zone_centers_file = os.path.join(script_dir, "data", "zone_centers.json")
    with open(zone_centers_file, encoding="utf-8") as f:
    ...
    print(
        f"Placed {stats['ocr'] + stats['interpolated'] + stats['zone']}/{len(employees)} employees"
    )
    ...
    for density, emp in employees_with_density:
        ...
        for dist in distances_to_try:
            for angle_deg in angle_preferences:
                ...
                if not collision:
                    label_data.append(
                        (person_id, x_canvas, y_canvas, label_x, label_y, method, name, room)
                    )
    ...
    # Draw lines
    for _person_id, x, y, label_x, label_y, _method, _name, _room in label_data:
        dx = label_x - x
        dy = label_y - y
    ...
    for person_id, x, y, _label_x, _label_y, _method, _name, _room in label_data:
    ...
    # Draw labels
    for _person_id, _x, _y, label_x, label_y, _method, name, room in label_data:
        name_bbox = draw.textbbox((0, 0), name, font=font_name)
        name_width = name_bbox[2] - name_bbox[0]
        name_height = name_bbox[3] - name_bbox[1]
